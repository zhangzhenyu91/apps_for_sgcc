<h1 align="center">国网一线应用部署</h1>

<p align="center">
  <img alt="GitHub last commit" src="https://img.shields.io/github/last-commit/zhangzhenyu91/apps_for_sgcc">
  <img alt="GitHub commit activity" src="https://img.shields.io/github/commit-activity/t/zhangzhenyu91/apps_for_sgcc">
  <img alt="GitHub contributors" src="https://img.shields.io/github/contributors-anon/zhangzhenyu91/apps_for_sgcc">
  <img alt="GitHub License" src="https://img.shields.io/github/license/zhangzhenyu91/apps_for_sgcc">
</p>
<hr>

## ✨ 简介

项目是使用 [VitePress](https://vitepress.dev/zh/) 搭建的教程文档，旨在寻找国网一线生产办公环境可用、好用的软件、Docker 应用（前提是在班组内网计算机里有可执行性），并集部署方式、使用中的难点等整合成文档供同事们参考阅读。

## 📧 欢迎交流

作者没有专业学习过，兴趣爱好使然，欢迎各行各业有兴趣的人一起交流，有任何需要均可联系作者。

联系邮箱：badzhang@j1net.org

## 💻 兼容性

目前作者班组办公所用的计算机：

> 测试机型：联想 开天 M99h G1t（海光 C86 3250）
>
> 测试系统：银河麒麟 V10 SP1 2503
>
> 网络环境：内网

## 🙏 致谢

感谢以下开发者、开发团队创造的好用的软件和项目（无先后顺序）：

[VitePress](https://vitepress.dev/zh/)、[Cloudreve](https://cloudreve.org)、[RustFS](https://rustfs.com.cn)、[OpenList](https://doc.oplist.org)、[PandaWiki](https://pandawiki.docs.baizhi.cloud)、[DzzOffice（小胡团队）](https://xiaohu.apifox.cn/52250940f0)、[1Panel](https://1panel.cn)